...
class Canvas(QWidget):
        ...
        for node in reversed(self.nodes):
            if node.contains(pos):
                ...
                if menu.clickedButton() == delete_button:
                    ...
                    self.connections = [
                        c for c in self.connections if c[0] != node and c[1] != node
                    ]
                    self.update()
                return
    ...
    def paintEvent(self, event):
        ...
        for start, end in self.connections:
            ...
            if isinstance(start, QPoint):  # Markerpunkt → T-Verzweigung
    ...
    def mousePressEvent(self, event: QMouseEvent):
        ...
        if self.creating_link:
            virtual_point = self.find_virtual_connection_point(pos)
            if virtual_point:
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        if self.dragging_node:
            ...
            snap_threshold = 10
            this_center = new_pos + QPoint(
                self.dragging_node.size.width() // 2,
                self.dragging_node.size.height() // 2
            )

            for other in self.nodes:
                if other is self.dragging_node:
                    continue
                other_center = other.position + QPoint(
                    other.size.width() // 2, other.size.height() // 2
                )
                if abs(this_center.x() - other_center.x()) < snap_threshold:
                    new_pos.setX(other.position.x())
                    self.snap_lines.append(
                        (QPoint(other_center.x(), 0), QPoint(other_center.x(), self.height()))
                    )
                if abs(this_center.y() - other_center.y()) < snap_threshold:
                    new_pos.setY(other.position.y())
                    self.snap_lines.append(
                        (QPoint(0, other_center.y()), QPoint(self.width(), other_center.y()))
                    )
